...
class AdaBIGGAN(nn.Module):
    def __init__(self, generator, dataset_size, embed_dim=120, shared_embed_dim=128, cond_embed_dim=20, embedding_init="zero"):
        ...
        if embedding_init == "zero":
            self.embeddings.from_pretrained(torch.zeros(
                dataset_size, embed_dim), freeze=False)
        ...
        self.linear = nn.Embedding(3, shared_embed_dim)
    ...
    def forward(self, z, y):
        ...
        h = h*self.bsa_linear_scale.view(1, -1, 1, 1) + \
            self.bsa_linear_bias.view(1, -1, 1, 1)
    ...
    def set_training_parameters(self):
        ...
        named_params_requires_grad.update(
            self.calss_conditional_embeddings_params())
