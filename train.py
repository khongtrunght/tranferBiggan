...
def argparse_setup():
    ...
    parser.add_argument('--model', type=str,
                        default="biggan128-ada", help="model. biggan128-ada")
...
def setup_optimizer(model, lr_g_batch_stat, lr_g_linear, lr_bsa_linear, lr_embed, lr_class_cond_embed, step, step_facter=0.1):
    ...
    params = []
    params.append(
        {"params": list(model.batch_stat_gen_params().values()), "lr": lr_g_batch_stat})
    params.append(
        {"params": list(model.linear_gen_params().values()), "lr": lr_g_linear})
    params.append(
        {"params": list(model.bsa_linear_params().values()), "lr": lr_bsa_linear})
    params.append(
        {"params": list(model.emebeddings_params().values()), "lr": lr_embed})
    params.append({"params": list(
        model.calss_conditional_embeddings_params().values()), "lr": lr_class_cond_embed})
...
def main(args):
    ...
    model = setup_model(args.model, dataset_size=dataset_size, resume=args.resume,
    ...
    while(True):
        ...
        for data in dataloader:
            ...
            real_label_embeddings = model.linear(real_label)
            ...
            # forward
            img_generated = model(
                embeddings+embeddings_eps, real_label_embeddings)
